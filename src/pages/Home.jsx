...
export default function Home() {
  ...
  const isSteakburgerPlatters = categories[activeCategory] === 'Steakburger Platters'
            ...
            {menuLayout === 'grid' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {currentItems.map((item, index) => (
                  <div key={`grid-${index}`} className="glass rounded-3xl p-5 hover-card group">
                    <div className="flex items-stretch gap-4">
                      ...
                      <div className="w-28 sm:w-36 md:w-40 shrink-0 flex flex-col items-end justify-between ml-auto">
                        ...
                        <div className="absolute top-2 left-2">
                          {(isInCart(item) || highlightedItems.has(index)) && (
                            <span className="badge-glass text-primary-300 text-[11px] backdrop-blur-md">
                              {isInCart(item) ? `${getCartItemQuantity(item)} in cart` : 'Popular'}
                            </span>
                          )}
                        </div>
                      </div>
                        ...
                        {!isInCart(item) ? (
                          <button onClick={() => isSteakburgerPlatters ? openModifierModal(item) : addToCart(item)} className="btn-primary text-xs py-2 px-4 inline-flex items-center gap-1.5">
                  ...
                  <div key={`compact-${index}`} className="glass rounded-2xl overflow-hidden hover-card group">
                    ...
                    <div className="p-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div>
                          {!isInCart(item) ? (
                            <button onClick={() => isSteakburgerPlatters ? openModifierModal(item) : addToCart(item)} className="w-7 h-7 rounded-lg bg-primary-500 text-white flex items-center justify-center hover:bg-primary-600 transition-colors">
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"/></svg>
                            </button>
                          ) : (
